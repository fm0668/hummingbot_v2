...
from hummingbot.strategy_v2.executors.hedge_grid_executor.data_types import GridExecutorConfig  # 导入网格执行器配置
...
class GridStrikeConfig(ControllerConfigBase):
    ...
    """
    controller_type: str = "hedge_grid_strike"  # 控制器类型
    controller_name: str = "hedge_grid_strike"  # 控制器名称
...
class GridStrike(ControllerBase):
    ...
    def create_hedge_executor(self, primary_executor_id: str) -> ExecutorAction:
        ...
        primary_config = GridExecutorConfig(
            ...
            connector_name=self.config.hedge_connector_name,  # 对冲连接器名称
            ...
            side=TradeType.SELL if self.config.side == TradeType.BUY else TradeType.BUY,  # 交易方向相反
            ...
            is_primary=False,  # 非主执行器
            hedge_mode=True,  # 启用对冲模式
            hedge_connector_name=self.config.connector_name,  # 对冲连接器为主连接器
            primary_account=self.config.hedge_account_api_key,  # 交换账号标识
            hedge_account=self.config.primary_account_api_key,  # 交换账号标识
            hedge_executor_id=primary_executor_id,  # 主执行器ID
    ...
    def on_executor_created(self, executor_info: ExecutorInfo):
        ...
        if (self.config.enable_hedge and self.config.hedge_connector_name and 
            hasattr(config, 'is_primary') and config.is_primary):
            # 保存主执行器引用
    ...
    def on_executor_updated(self, executor_info: ExecutorInfo):
        ...
        config = executor_info.config
        if (hasattr(config, 'hedge_mode') and config.hedge_mode and 
            hasattr(config, 'is_primary') and not config.is_primary and
            hasattr(config, 'hedge_executor_id') and config.hedge_executor_id is not None):
            # 设置对冲执行器引用
    ...
    def to_format_status(self) -> List[str]:
        ...
        for level in self.active_executors():
            # 判断是否为对冲执行器
            is_hedge = False
            if hasattr(level.config, 'hedge_mode') and level.config.hedge_mode:
                is_hedge = hasattr(level.config, 'is_primary') and not level.config.is_primary
        ...
        if self.config.enable_hedge and len(self.active_executors()) > 1:
            ...
            for level in self.active_executors():
                if hasattr(level.config, 'hedge_mode') and level.config.hedge_mode:
                    if hasattr(level.config, 'is_primary') and level.config.is_primary:
